...
namespace Emikon.Parkon.UI.Win.test.Views.Store_ProductBlockCollectionView{
    public partial class Store_ProductBlockCollectionView : XtraUserControl {
        public Store_ProductBlockCollectionView() {
            InitializeComponent();
			if(!mvvmContext.IsDesignMode)
                InitBindings();
        }
        void InitBindings() {
            var fluentAPI = mvvmContext.OfType<Emikon.Parkon.UI.Win.test.ViewModels.Store_ProductBlockCollectionViewModel>();
            ...
            fluentAPI.WithEvent<GridView, FocusedRowObjectChangedEventArgs>(gridView, "FocusedRowObjectChanged")
                .SetBinding(x => x.SelectedEntity,
                    args => args.Row as Emikon.Parkon.Model.Store_ProductBlock,
